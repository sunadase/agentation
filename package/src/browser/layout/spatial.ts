 ...
 */
function collectDOMCandidates(environment: RuntimeEnvironment): Candidate[] {
  const doc = environment.document;
  const main = doc.querySelector("main") || doc.body;
  const results: Candidate[] = [];

  const topLevel = Array.from(main.children) as HTMLElement[];
  const roots = main !== doc.body && topLevel.length < 3
    ? Array.from(doc.body.children) as HTMLElement[]
    : topLevel;
 ...
 */
function toViewportEdges(environment: RuntimeEnvironment, r: SectionRect) {
  const scrollY = environment.scrollY;
...
export function getSpatialContext(
  ...
  const candidates = siblings
    ? explicitToCandidates(environment, siblings)
    : collectDOMCandidates(environment);
...
export function getPageLayout(
  ...
  const roleMain = doc.querySelector("[role='main']") as HTMLElement | null;
  if (roleMain) addCandidate(roleMain);

  // Walk 3 levels deep from body to catch framework wrappers
  for (const l1 of Array.from(doc.body.children) as HTMLElement[]) {
    addCandidate(l1);
    if (l1.children) {
 ...
 */
export function getElementCSSContext(
  environment: RuntimeEnvironment,
  selector: string,
): ElementCSSContext | null {
  const el = environment.document.querySelector(selector) as HTMLElement | null;
